...
public class L048 {
  ...
  public static class myHeap {
    ...
    private boolean compareTo(int child, int par) {
      if (isMaxheap) return this.arr.get(child) > this.arr.get(par); // RETURN TRUE AS WE NEED TO UPDATE THE PARENT IN THE
      // CALLER FN
      else return this.arr.get(child) < this.arr.get(par);
    }
    ...
    private void downHeapify(int idx) {
      ...
      if (lChild < this.arr.size() && compareTo(lChild, pi)) pi = lChild;
      if (rChild < this.arr.size() && compareTo(rChild, pi)) pi = rChild;
    ...
    public int height(int idx) { // in terms of edges, thats why for a leaf height is 0.
      if (idx >= this.arr.size()) return -1; // for a null left/right child,return -1
  ...
  public static void downHeapify_(int idx, int limit, ArrayList<Integer> arr) {
    ...
    if (lChild <= limit && arr.get(lChild) > arr.get(maxIdx)) maxIdx = lChild;
    if (rChild <= limit && arr.get(rChild) > arr.get(maxIdx)) maxIdx = rChild;
